#!/usr/bin/env python3
"""
Causal MS-TCN for real-time work-state classification.

Key difference from standard MS-TCN:
  - DilatedResidualLayer uses CAUSAL (left-only) padding so the prediction
    at frame t depends only on frames <= t.  This simulates real-time inference.

Architecture: MultiStageModel → SingleStageModel → CausalDilatedResidualLayer
"""
...
class CausalDilatedResidualLayer(nn.Module):
    """Dilated residual layer with CAUSAL padding (no future leakage)."""

    def __init__(self, dilation, in_channels, out_channels):
        super(CausalDilatedResidualLayer, self).__init__()
        # For kernel_size=3 and causal: pad left by (kernel_size-1)*dilation = 2*dilation
        # No padding on the right
        self.causal_pad = 2 * dilation
    ...
    def forward(self, x, mask):
        # Left-pad only: (left, right) = (causal_pad, 0)
...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, device,
        ...
        If `val_batch_gen` is provided, runs validation each epoch and writes
        a CSV log to `log_path` with columns:
            epoch, train_loss, train_acc, val_loss, val_acc, time_s
        Backwards-compatible: if val_batch_gen is None, behaves exactly as
        the original trainer (no val pass, no CSV).
        """
        ...
        for epoch in range(num_epochs):
            ...
            # Validation pass (no grad, no smoothness term in the metric for clarity —
            # but include it so val_loss is directly comparable to train_loss)
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file,
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                ...
                # Softmax probabilities from the last stage: shape (num_classes, T)
                ...
                # Save frame-level label predictions (same as before)
                f_ptr = open(results_dir + "/" + f_name, "w")
                f_ptr.write("### Frame level recognition: ###\n")
                f_ptr.write(' '.join(recognition))
                f_ptr.close()

                # Save softmax probabilities: shape (num_classes, T_subsampled)
                # Expand back to full frame rate to match ground truth length
                if sample_rate > 1:
