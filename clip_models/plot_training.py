...
Generates two side-by-side plots:
  - Loss curves   : train + val loss over epochs
  - Accuracy curves: train + val accuracy over epochs

Works for any CSV log with columns:
  epoch, train_loss, train_acc, val_loss, val_acc, [lr, time_s ignored]

Outputs a PNG next to the CSV with the same basename.
